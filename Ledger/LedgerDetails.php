    ...
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      <header class="demo-header mdl-layout__header mdl-color--white mdl-color--grey-100 mdl-color-text--grey-600" style="background-color: rgb(63, 81, 181); color: #ffffff;">
        ...
        <div class="mdl-layout__header-row" style="background-color: rgb(63, 81, 181); color: #ffffff;">
          ...
          <span class="mdl-layout-title">Ledger Details</span>
...
<!--Add help here -->
        ...
        <div class="mdl-grid demo-content">
          <div class="demo-card mdl-cell mdl-cell--12-col mdl-grid">
            <div style="width: 40%; float: left; padding-left: 10px;">
              <h5 class="title-text">Ledger for</h5>
            </div>
            ...
            <div style="width: 40%; float: left; padding-left: 10px;">
							...
							<?php
								$sql = "SELECT SUM(Payment_Received) AS sum FROM ledger WHERE Date='{$date}'";
								$result = $mysqli->query($sql);
								$result = $result->fetch_assoc();
								echo $result['sum'];
							?>
            ...
            <div style="width: 50%; margin-right: 0px; float:right; text-align: right;">
							...
							<?php
								$sql = "SELECT SUM(Bill_Amount) AS sum FROM ledger WHERE Date='{$date}'";
								$result = $mysqli->query($sql);
								$result = $result->fetch_assoc();
								echo $result['sum'];
              ?>
    ...
    if ($result->num_rows <=0){
      ...
      $result = $mysqli->query($sql);
      echo '<p style="font-weight: 400; text-align: center; padding-top: 32px;">No payment entries for this day.</p><br>';
    }
    else{
  ...
  ?>
    ...
    if ($result->num_rows <=0){
      ...
      $result = $mysqli->query($sql);
      echo '<p style="font-weight: 400; text-align: center; padding-top: 32px;">No bill entries for this day.</p><br>';
    }
    else{
      echo '<div class="table-responsive-vertical shadow-z-1">
    	  <!-- Table starts here -->
